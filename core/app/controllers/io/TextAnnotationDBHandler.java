...
public class TextAnnotationDBHandler {
	...
	public void updateTextAnnotation(TextAnnotation ta) {
		...
		try {
			...
			int id = ta.getTokenizedText().hashCode();
			byte[] bytes = serialize(ta);
			PreparedStatement stmt = connection.prepareStatement("update sentences set ta = ? where id = ?");
	...
	public void removeTextAnnotation(TextAnnotation ta) {
		try {
			...
			PreparedStatement stmt = connection.prepareStatement("delete from sentencesToDataset where sentenceId = ?");
			stmt.setInt(1, id);
			stmt.execute();
			stmt.close();
